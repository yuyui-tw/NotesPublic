...
> - 2026/7/1 下午12:58:07: Created - "[Content of workflows/Note Formatter.md]
...
[/Content]"
>   > [!note]- @workflows/Note Formatter.md
>   > ```
>   > Create a robust Obsidian workflow for note formatting.
>   > 
>   > Workflow name:
...
nodes:
  ...
  - id: prompt-llm
    type: command
    prompt: |
      請將以下內容整理成 Obsidian Markdown 筆記，保留重要術語、公式與原意，使用清楚段落與條列，不要新增例題。
  ...
  - id: offer-actions
    type: dialog
    title: "What do you want to do with this note?"
    options: "[\"Replace current selection\",\"Append below current note\",\"Save as a new note\"]"
    saveTo: "userChoice"
  - id: handle-replace
    type: script
    code: |
      return ""; # Placeholder for replace functionality
      
  - id: handle-append
    type: script
    code: |
      return ""; # Placeholder for append functionality
      
  - id: handle-save-new-note
    type: note
  ...
  - id: generateFilename
    ...
    code: |
      ...
      // Try to extract a useful title from the content if possible
      if(typeof {{content}} === 'string') {
        // Placeholder for extracting title logic (improve as needed)
        title = "Formatted Note"; // Default filename
      }
